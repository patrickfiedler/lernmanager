# Lernmanager - Ideas & Future Plans

## High Priority

- 502 error when I try to upload a file
- error logging functionality

## Features

- Logging functionality: track number of users and page views
- logging functionality #2: track student activities in action log
- Student progress reports as PDF file per class: human readable format for quick overview
- student progress reports as PDF file per student: information from class progress report + student's individual activity log
- Add regular class dates for each class (schedule)

## Improvements

- student view: show only the current (or the first) subtask of the active task
- (low priority, optional) student view: show visual learning map of open tasks and how they connect to each other; for the moment only for informational purposes
- admin view: assign particular subtasks to classes and to students
- admin view: when batch-importing students, add the url of the app to each line, along with username and password (either hardcode lernen.mrfiedler.de, or maybe read from configuration or HTML headers?)
- admin view: allow individual students to see all available tasks, but default to the current behaviour (students see only the active task)

## Bugs

- Fix task sorting: should be 1-2-3-10, not 1-10-2-3 (alphabetical vs numerical)
- Class assessment: make it obvious if data has been saved for a day (currently unclear - shows default 2/3 points for all dates)
- (high priority) student attendance and evaluation page -> completely rewrite this page: ask me for admin's user experience and keep asking questions

## Notes

-
